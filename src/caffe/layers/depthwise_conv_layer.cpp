...
void DepthwiseConvolutionLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  if (conv_param.has_group()) {
    CHECK_EQ(conv_param.group(), conv_param.num_output()) 
        << "The number of group must eaual to the number of num_output."; 
  } else {
    this->layer_param_.mutable_convolution_param()->set_group(conv_param.num_output());
  } 
