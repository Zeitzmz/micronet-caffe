...
template <>
void caffe_gpu_gemmex<float>(const CBLAS_TRANSPOSE TransA,
  ...
  CUBLAS_CHECK(cublasGemmEx(Caffe::cublas_handle(), cuTransB, cuTransA,
      N, M, K, &alpha, B, CUDA_R_16F, ldb, A, CUDA_R_16F, lda, &beta, C, CUDA_R_16F, N, CUDA_R_16F, CUBLAS_GEMM_DEFAULT));
}

template <>
void caffe_gpu_gemmex<double>(const CBLAS_TRANSPOSE TransA,
    const CBLAS_TRANSPOSE TransB, const int M, const int N, const int K,
